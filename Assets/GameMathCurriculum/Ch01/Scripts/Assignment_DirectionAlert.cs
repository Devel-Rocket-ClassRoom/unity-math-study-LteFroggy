...
public class Assignment_DirectionAlert : MonoBehaviour
{
    ...
    {
        // 상대까지의 벡터 탐색
        Vector3 vecToEnemy = enemy.position - transform.position;
        // 감지 거리 내에 없으면 None
        if (vecToEnemy.sqrMagnitude >= Mathf.Pow(alertRange, 2)) { return Direction.None; }
        // 정규화
        vecToEnemy.Normalize();
        
        // dotProduct 계산
        float dotProduct = Vector3.Dot(vecToEnemy, transform.forward);
        // 전방 90도 이내라면 front
        if (dotProduct >= Mathf.Cos(45 * Mathf.Deg2Rad)) {  return Direction.Front; }
        // 후방 90도 이내라면 back
        if (dotProduct <= MathF.Cos(135 * Mathf.Deg2Rad)) { return Direction.Back; }
        
        // 외적 체크
        Vector3 crossProduct = Vector3.Cross(transform.forward, vecToEnemy);
        // 0보다 크면 우측
        if (crossProduct.y > 0) { return Direction.Right; }
        // 0보다 작으면 좌측
        else if ( crossProduct.y < 0 ) { return Direction.Left; }
