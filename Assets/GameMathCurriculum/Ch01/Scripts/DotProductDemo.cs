...
public class DotProductDemo : MonoBehaviour
{
    ...
    private bool CheckInSight(Transform targetTransform)
    {
        ...
        // 벡터의 거리가 내가 보고자 하는 시야보다 크다면 볼 수 없음
        if (vectorToTarget.sqrMagnitude >= viewDistance * viewDistance) {
            return false;
        }
        ...
        // dotProduct로 float값 받기
        dotProductValue = Vector3.Dot(transform.forward, vectorToTarget);
