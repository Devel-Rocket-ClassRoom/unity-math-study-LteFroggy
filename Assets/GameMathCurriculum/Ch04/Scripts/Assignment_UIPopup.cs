...
public class Assignment_UIPopup : MonoBehaviour
{
    ...
    private Vector3 _popupScale = Vector3.one * 3;
    ...
    private void UpdateAnimation()
    {
        ...
        {
            // 시간동안 Scale과 Alpha를 0 -> 1로 보간
            // 보간에는 Lerp 사용
            t = EaseIn(t);
            float lerpResult = Mathf.Lerp(0, 1, t);
            Vector3 scale = new Vector3(lerpResult, lerpResult, lerpResult);
            scale *= 3;
            popupPanel.localScale = scale;
            canvasGroup.alpha = lerpResult;

            if (t >= 1f)
            {
                currentState = PopupState.Visible;
                popupPanel.localScale = _popupScale;
                canvasGroup.alpha = 1f;
            }
        }
        else if (currentState == PopupState.Hiding)
        {
            // 시간동안 Scale과 Alpha를 1 -> 0으로 보간
            // 보간에는 Lerp 사용
            t = EaseOut(t);
            float lerpResult = Mathf.Lerp(1, 0, t);
            Vector3 scale = new Vector3(lerpResult, lerpResult, lerpResult);
            scale *= 3;
            popupPanel.localScale = scale;
            canvasGroup.alpha = lerpResult;
