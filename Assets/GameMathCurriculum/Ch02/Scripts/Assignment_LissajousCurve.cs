...
public class Assignment_LissajousCurve : MonoBehaviour
{
    ...
    private Vector3 CalculateLissajousPosition(float time)
    {
        ...
        // TODO: 위 공식을 구현하세요
        Vector3 xVariant = Vector3.right * amplitudeX * Mathf.Sin(2f * frequencyX * Mathf.PI * Time.time + (phaseX * Mathf.Deg2Rad));
        Vector3 zVariant = Vector3.forward * amplitudeZ * Mathf.Sin(2f * frequencyZ * Mathf.PI *  Time.time + (phaseZ * Mathf.Deg2Rad));
        
        return initialPosition + xVariant + zVariant;
    }
