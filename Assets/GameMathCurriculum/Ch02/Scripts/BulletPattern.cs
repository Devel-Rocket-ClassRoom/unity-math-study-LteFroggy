...
public class BulletPattern : MonoBehaviour
{
    ...
    private void FireBulletPattern()
    {
        for (int i = 0; i < bulletCount; i++)  {
            float shotAngleDegree = (i * angleSpacing + currentRotationOffset) % 360f;
            float shotAngleRadian = shotAngleDegree * Mathf.Deg2Rad;
            
            Vector3 bulletDirection = new Vector3(Mathf.Sin(shotAngleRadian), 0f,  Mathf.Cos(shotAngleRadian)).normalized;
