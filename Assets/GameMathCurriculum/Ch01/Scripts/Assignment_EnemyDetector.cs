...
public class Assignment_EnemyDetector : MonoBehaviour
{
    ...
    {
        // 나와 적까지의 Vector 보기
        Vector3 directionVector = enemy.position - transform.position;
        // 거리 확인
        if (directionVector.sqrMagnitude >= detectionRange *  detectionRange) { return false; }
        // 정규화
        directionVector.Normalize();
        // DotProduct 구하기 -> cos 값
        float dotProduct = Vector3.Dot(transform.forward, directionVector);
        
        // fov와 cos값과 비교하여 반환
        return Mathf.Cos(detectionFOV / 2f) <= dotProduct;
    }
